...
public class parserHTML {
	...
	public static String close="app.closeDialog(id)";
	...
	public static String tableCSS = "css\\tableCSS.css";
	public static String tempTableCSS = "css\\tableCSS.css";
	public static String hrefTableCSS = "css/tableCSS.css";
	...
	public static void addmenu(ArrayList<pMenu> bar){
		...
		//System.out.println("Arraylist vuoto?" +bar.isEmpty());
	...
	public static void parse_menu(ArrayList<pMenu> bar){
		...
		for(i=0;i<bar.size();i++){
				...
				ul.appendChild(li);
				li.appendChild(doc.createTextNode("- " + bar.get(i).getText()  + ": "));
				li.setAttribute("onclick", "insert_menu(this)");
				li.setAttribute("id", bar.get(i).getName() );	
	...
	private static void parse_menu_item(Element tag_li, ArrayList<pMenuItem> pmenui){
		...
		for(i=0;i<pmenui.size();i++){
				...
				Element li = doc.createElement("li"); //System.out.println("<li>"); 
				ul.appendChild(li);
				li.appendChild(doc.createTextNode(pmenui.get(i).getText()) ); //System.out.println(pmenui.get(i).getName());
...
//			System.out.println(c.getClass().getName());
		...
		} else if(c instanceof pMenu || c instanceof pMenuItem) {
			return;
		}
	...
	public static void createPage(String title, Set<Set<String>> partition){
		try {
			...
			System.out.println("canello File Tabellllaaaaaaaaaaa");
	...
	private static void addFooter(){
		...
		buttonLeft.setAttribute("onClick","spostamentoFrecciaSinistra()");
		buttonRight.setAttribute("id","buttonRight");
		buttonRight.setAttribute("onClick","spostamentoFrecciaDestra()");
	...
	public static void addButton(pButton b){
		Element button = doc.createElement("button");
		button.setAttribute("type", "button");
		button.setAttribute("class", "button");
		button.setAttribute("id", b.getName());
		button.appendChild(doc.createTextNode(b.getLabel()));
		button.setAttribute("onClick", buttonEvent);	
		elements.put(b.getName(), button);
	...
	public static void addProgressBar(String name, int min, int max, int orientation) {
		...
		if(orientation==1) {
			//show progress bar vertically
		}
		progressBar.appendChild(doc.createTextNode(name));
		elements.put(name, progressBar);
	}
	
	public static void addTextArea(String text, String title, int rows, int columns) {
		...
		textArea.setAttribute("onkeyup", textEvent);
		textArea.appendChild(doc.createTextNode(text));
		elements.put(title, textArea);
	}
	
	
	public static void addPasswordField(String title, String text) {
...
//		add_component(dialog);
	...
	public static void addLabel(pLabel name,Element root){
		Element list = doc.createElement("DIV");
		Element label = doc.createElement("LABEL");
		label.setAttribute("id", name.getName());
		label.setAttribute("class", "label");
		Element font = null;
		
		if(name.getFont()!=null) {
			if(name.getFont().getStyle() == Font.BOLD)
				font = doc.createElement("b");
			if(name.getFont().getStyle() == Font.ITALIC)
				font = doc.createElement("i");
			font.appendChild(doc.createTextNode(name.getText()));
			label.appendChild(font);
		}
		else{
			label.appendChild(doc.createTextNode(name.getText()));
		}
		list.appendChild(label);
		if(root != null)
			root.appendChild(label);
		else 		
			elements.put(name.getName(), label);
	}
...
//			System.out.println("root null");
	...
	public static void addTextField(pTextField p, Element root){
		Element list = doc.createElement("DIV");
		list.setAttribute("class", "areaditesto");
		Element label = doc.createElement("LABEL");
		Element textField = doc.createElement("INPUT");
		textField.setAttribute("type", "text");
		textField.setAttribute("placeholder", p.getText());
		textField.setAttribute("id", p.getName());
		textField.setAttribute("onkeyup", textEvent);
		textField.setAttribute("onfocus", "app.fire(id)");
		label.appendChild(textField);
		list.appendChild(label);
		if(root != null)
			root.appendChild(list);
		else 
			elements.put(p.getName(), textField);
	}	
...
//					System.out.println("\t"+con+++")"+element);
	...
	public static void setPath(String path) {
		tableCSS = tempTableCSS;
		tableCSS = path+tableCSS;
	}
